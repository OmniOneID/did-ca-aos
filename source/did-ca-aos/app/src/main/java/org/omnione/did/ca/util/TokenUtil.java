...
public final class TokenUtil {
    ...
    public static String createHashWalletToken(String walletTokenDataStr,
                                               Context context,
                                               String walletApiGwBaseUrl)
            throws WalletException, UtilityException, WalletCoreException,
                   ExecutionException, InterruptedException {
        WalletApi walletApi = WalletApi.getInstance(context);
        WalletTokenData walletTokenData =
                MessageUtil.deserialize(walletTokenDataStr, WalletTokenData.class);
        String resultNonce = walletApi.createNonceForWalletToken(walletApiGwBaseUrl, walletTokenData);
        String walletToken = walletTokenData.toJson() + resultNonce;
        walletToken = Base16.toHex(
                DigestUtils.getDigest(walletToken.getBytes(), DigestEnum.DIGEST_ENUM.SHA_256));
        AppLog.d(TAG, "walletToken by CA : " + walletToken
                + " / " + walletTokenData.getSeed().getPurpose().toString());
        return walletToken;
    }

    public static String createServerToken(String std, String ecdh,
                                           byte[] clientNonce, EcKeyPair dhKeyPair)
        ...
        String cipherValue = accEcdh.getCipher().getValue();
        String[] cipher = cipherValue.split("-");
        if (cipher.length != 3) {
            throw new IllegalStateException("Unexpected ECDH cipher format: " + cipherValue);
        }
        ...
        byte[] plain = CryptoUtils.decrypt(
                MultibaseUtils.decode(encStd), info, sessKey, MultibaseUtils.decode(iv));
        String serverToken = MultibaseUtils.encode(
                MultibaseType.MULTIBASE_TYPE.BASE_58_BTC,
                DigestUtils.getDigest(new String(plain).getBytes(), DigestEnum.DIGEST_ENUM.SHA_256));
        AppLog.d(TAG, "serverToken by CA : " + serverToken);
        return serverToken;
    }

    private static byte[] createDhSecret(byte[] clientNonce, AccEcdh accEcdh, EcKeyPair dhKeyPair)
            throws UtilityException {
        ...
        AppLog.d(TAG, "getECDHSharedSecret client private key " + multibasePrivKey + " / " + privKey.length);
        AppLog.d(TAG, "getECDHSharedSecret server public key " + multibasePubKey
                + " / " + MultibaseUtils.decode(multibasePubKey).length);

        byte[] serverPubKey = MultibaseUtils.decode(multibasePubKey);

        byte[] secretKey = CryptoUtils.generateSharedSecret(
                EcType.EC_TYPE.SECP256_R1, privKey, serverPubKey);

        byte[] serverNonce = MultibaseUtils.decode(accEcdh.getServerNonce());
        byte[] mergedNonce = mergeNonce(clientNonce, serverNonce);
        return mergeSharedSecretAndNonce(secretKey, mergedNonce,
                SymmetricCipherType.SYMMETRIC_CIPHER_TYPE.AES256CBC);
    }

    private static byte[] mergeNonce(byte[] clientNonce, byte[] serverNonce) throws UtilityException {
    ...
    private static byte[] mergeSharedSecretAndNonce(
            byte[] sharedSecret, byte[] nonce,
            SymmetricCipherType.SYMMETRIC_CIPHER_TYPE cipherType) throws UtilityException {
